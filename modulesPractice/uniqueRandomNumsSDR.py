...
def unique_random_ints(how_many, max_num):
    ...
    retries = 0
    unique_ints = []
    while len(unique_ints) < how_many:
...
if __name__ == '__main__':
    ...
    max_num = int(input())
    random.seed(seed)
    unique_ints = unique_random_ints(how_many, max_num)

    formatted_ints = ' '.join(str(i) for i in unique_ints)
    print(f'{formatted_ints} retries={retries}' )
